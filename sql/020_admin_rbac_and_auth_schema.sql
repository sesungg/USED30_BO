-- used30_bo admin RBAC/auth schema
-- Dependencies:
-- - Requires 010_user_member_schema.sql to be applied first.
-- - BO access must be resolved from these tables, not from users.role.
...
CREATE INDEX idx_admin_user_roles_active
  ON admin_user_roles (user_id, role_code)
  WHERE revoked_at IS NULL;

CREATE UNIQUE INDEX uq_admin_user_primary_role
...
CREATE INDEX idx_admin_login_logs_user
  ON admin_login_logs (user_id, created_at DESC);

CREATE INDEX idx_admin_login_logs_email
  ON admin_login_logs (email_snapshot, created_at DESC);

CREATE VIEW v_admin_effective_permissions AS
SELECT
  aur.user_id,
